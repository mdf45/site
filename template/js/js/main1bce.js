...
$(document).on("click", ".rating_field .stars img", function(){
	var rate = $(this).data("rate");
	$("input[name='rating_rate']").val(rate);
	$(".form .control_wrapper .rating_field .stars img").each(function(){
		if ($(this).data("rate") <= rate) {
			$(this).attr("src", "assets/templates/mega/svg/star_1.svg");
		} else {
			$(this).attr("src", "assets/templates/mega/svg/star_2.svg");
		}
	})
})
